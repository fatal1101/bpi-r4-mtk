...
#		radio_rx_antenna="$(uci show wireless.${radio} | grep rxantenna | cut -d '=' -f 2 | tr -d "'")"
		radio_tx_antenna="$(uci show wireless.${radio} | grep txpower | cut -d '=' -f 2 | tr -d "'")"
		radio_rx_antenna="$(uci show wireless.${radio} | grep txpower | cut -d '=' -f 2 | tr -d "'")"
		antenna_mask="$(iw phy ${phy} info | grep "wiphy radio" -A 21 | grep "Idx $radio_idx" -A 2 | grep "antenna_mask" | cut -d ' ' -f 2 | cut -d 'x' -f 2)"
		if [ -n "$antenna_mask" ]; then
		    antenna_mask=$((0x$antenna_mask))
		else
		    echo "Warning: antenna_mask not found for radio $radio and radio_idx $radio_idx on $phy" >> /tmp/test
			echo "radio_tx_antenna $radio_tx_antenna and radio_rx_antenna $radio_rx_antenna" >> /tmp/test2
		    continue
		fi
